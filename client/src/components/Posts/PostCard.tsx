...
export function PostCard({ post }: { post: FeedPost }) {
  ...
  return (
    <article className="md:border md:border-neutral-800 md:rounded-lg overflow-hidden md:max-w-[470px] mx-auto bg-black">
      {/* Header */}
      <div className="flex items-center justify-between px-3 md:px-4 py-2.5">
        <Link href={`/u/${post.username}`} className="flex items-center gap-3 group">
            ...
            <div className="flex flex-col leading-tight">
              <span className="text-sm font-semibold flex items-center gap-1">
                {post.username}
                {post.isVerified && (
                  <svg className="w-3 h-3 fill-ig-primary" viewBox="0 0 12 12">
                    <path d="M6 0l1.4 1.4L9.4 0.4l1 2 2.2 0.4-0.4 2.2L12.4 6 11 7.4l1 2-2 1-0.4 2.2-2.2-0.4L6 12 4.6 10.6 2.6 11.6l-1-2-2.2-0.4L-0.4 6-2 4.6-1 2.6-2.2 0.4 0 0 1.4-1.4z" />
                  </svg>
                )}
              </span>
        ...
        <button className="p-1 hover:opacity-70" aria-label="Options">
      ...
      >
        ...
        >
          {post.media.map((m) => (
            <div
              key={m.id}
              className="snap-center shrink-0 w-full aspect-square flex items-center justify-center"
            >
              {m.type === "video" ? (
                <video
                  src={m.url}
              ...
              {post.media.map((_, i) => (
                <span
                  ...
                  className={cn(
                    ...
                    i === slide ? "bg-blue-500" : "bg-white/50"
                  )}
                />
              ))}
            </div>
        ...
        {showHeart && (
          <Heart
            size={96}
            className="heart-burst absolute inset-0 m-auto fill-white text-white drop-shadow-lg pointer-events-none"
          />
        )}
      </div>
      ...
      <div className="px-3 md:px-4 pt-2 flex items-center justify-between">
        <div className="flex gap-3">
          <button
            onClick={() => toggleLike()}
            className="hover:opacity-60 transition active:scale-90"
            aria-label="Like"
          >
            <Heart
              size={26}
              className={cn(liked ? "fill-ig-danger text-ig-danger" : "")}
            />
          </button>
          <Link href={`/p/${post.id}`} className="hover:opacity-60" aria-label="Comment">
              <MessageCircle size={26} className="-scale-x-100" />
            </Link>
          <button className="hover:opacity-60" aria-label="Share">
            <Send size={26} />
          </button>
        </div>
        <button
          onClick={toggleSave}
          className="hover:opacity-60 active:scale-90"
          aria-label="Save"
        >
          <Bookmark size={26} className={cn(saved ? "fill-white text-white" : "")} />
        </button>
      </div>

      {/* Likes */}
      {likeCount > 0 && (
        <div className="px-3 md:px-4 pt-1.5 text-sm font-semibold">
      ...
      {post.caption && (
        <div className="px-3 md:px-4 pt-1 text-sm">
          <Link href={`/u/${post.username}`} className="font-semibold mr-2">{post.username}</Link>
      ...
      {/* Comment count link */}
      {post.commentCount > 0 && (
        <Link href={`/p/${post.id}`} className="block px-3 md:px-4 pt-1 text-sm text-ig-subtle">
            View all {formatCount(post.commentCount)} comments
          </Link>
      )}

      <div className="px-3 md:px-4 pt-1 text-xs text-ig-subtle uppercase">
      ...
      <form
        ...
        className="flex items-center gap-2 px-3 md:px-4 py-3 mt-2 border-t border-neutral-900"
      >
        ...
        {comment.trim() && (
          <button
            ...
            className="text-ig-primary text-sm font-semibold hover:text-white"
          >
            Post
          </button>
        )}
