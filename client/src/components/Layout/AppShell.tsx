...
function NavItem({
  ...
  const inner = (
    <div
      className={cn(
        "flex items-center gap-4 px-3 py-3 rounded-xl w-full transition-all duration-150",
        active
          ? "bg-neutral-900 font-bold"
          : "hover:bg-neutral-900/60 active:bg-neutral-800"
      )}
    >
      ...
      {!collapsed && (
        <span className={cn("text-base transition-all duration-150", active ? "font-bold" : "font-normal")}>
          {label}
        </span>
      )}
    </div>
  ...
  if (to) {
    return (
      <Link href={to} className="block pressable">
        {inner}
      </Link>
    );
  }
  return (
    <button onClick={onClick} className="block w-full text-left pressable">
      {inner}
    </button>
  );
}
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-black text-white flex">
      ...
      <aside
        className={cn(
          "hidden md:flex flex-col border-r border-neutral-800/70 fixed left-0 top-0 bottom-0 z-30 bg-black transition-all duration-200",
          collapsed ? "w-[72px] px-2" : "w-[244px] px-3",
          "py-6"
        )}
      >
        {/* Logo */}
        <Link href="/" className="px-3 mb-8 block pressable">
          {collapsed ? (
        ...
        <nav className="flex flex-col gap-0.5 flex-1">
          ...
          {user && (
            <NavItem
              ...
              icon={<Avatar src={user.avatarUrl} size={26} />}
              label="Profile"
              collapsed={collapsed}
            />
          )}
      ...
      {/* Mobile Top Bar */}
      <header className="md:hidden fixed top-0 left-0 right-0 z-20 bg-black/95 backdrop-blur-sm border-b border-neutral-900 safe-top">
        <div className="flex items-center justify-between px-4 h-12">
          <Link href="/" className="logo-font text-2xl">Instagram</Link>
          <div className="flex items-center gap-3">
            <Link href="/notifications" aria-label="Notifications" className="icon-btn p-1.5 rounded-full hover:bg-neutral-900">
              <Heart size={24} />
            </Link>
            <Link href="/messages" aria-label="Messages" className="icon-btn p-1.5 rounded-full hover:bg-neutral-900">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-20 bg-black/95 backdrop-blur-sm border-t border-neutral-900 safe-bottom">
        <div className="grid grid-cols-5 h-12">
          <Link href="/" className="flex items-center justify-center pressable">
            <Home size={26} className={cn("transition-all", location === "/" ? "fill-white" : "")} />
          </Link>
          <Link href="/search" className="flex items-center justify-center pressable">
            <Search size={26} className={cn(location === "/search" ? "stroke-[2.5]" : "")} />
          </Link>
          <button onClick={() => setCreating(true)} className="flex items-center justify-center pressable">
            <PlusSquare size={26} className={cn("transition-all", creating ? "opacity-60" : "")} />
          </button>
          <Link href="/reels" className="flex items-center justify-center pressable">
            <Film size={26} className={cn(location === "/reels" ? "fill-white" : "")} />
          </Link>
          {user ? (
            <Link href={`/u/${user.username}`} className="flex items-center justify-center pressable">
              <div className={cn(
                "rounded-full transition-all",
                location === `/u/${user.username}` ? "ring-2 ring-white ring-offset-1 ring-offset-black" : ""
              )}>
                <Avatar src={user.avatarUrl} size={26} />
              </div>
            </Link>
          ) : (
            <Link href="/login" className="flex items-center justify-center pressable">
              <Avatar size={26} />
            </Link>
          )}
        </div>
